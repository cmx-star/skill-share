...
#   2. resolve-plan-dir.sh — local workflow notes directory
#
...
#   Without --gate, or in non-gated mode, behavior is advisory only.
#
...
# past the gate. Per-field max preserves the single-format result
...
advisory_report() {
    if [ "$TOTAL" -eq 0 ]; then
        echo "[planning-with-files] No phases found in task_plan.md. Add phases before using completion checks."
        return
    fi
